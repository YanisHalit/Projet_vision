...
void test_swp_morpho_max_routine(int h, int w0){
    ...
    ** Y_bas_or_packed;
    ...
    zero_ui8matrix(T,  0-b, h-1+b, 0-b, w1-1+b);
    zero_ui8matrix(Y_bas_or_packed,  0-b, h-1+b, 0-b, w1-1+b);

    zero_ui8matrix(Y_bas_originale,         0, h-1, 0, w1-1);
    zero_ui8matrix(Y_P,         0, h-1, 0, w1-1);
    ...
    // display_ui8matrix(Y_bas_originale,         0, h-1, 0, w0-1, "%5d", "Y bas originale        ");
    ...
    c = compare_swp_ui8matrix(Y_bas_originale, 0, h-1, 0, w0-1, Y_bas        , "Y bas_swp                       ");
